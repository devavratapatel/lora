...
class DreamBoothDataset(Dataset):
    ...
    center_crop=False,
        ...
        # --- START OF THE FIX ---
        # First, get a list of all files, then filter for common image extensions.
        all_files = list(Path(instance_data_root).iterdir())
        image_paths = [
        ...
        if not image_paths:
            raise ValueError(f"No images with extensions .png, .jpg, .jpeg, or .webp found in {instance_data_root}")

        # Now, use the filtered 'image_paths' list for both operations.
        instance_images = [Image.open(path) for path in image_paths]
        image_hashes = [self.generate_image_hash(path) for path in image_paths]
        # --- END OF THE FIX ---
